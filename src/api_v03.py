...
BATCH_SIZE = 100
MAX_PAGES = 600
OUTFILE = "data/raw/data_filtered.parquet"
VIEW = "cats"
...
total_pages = math.ceil(meta["meta"]["count"] / LIMIT)
end_page = min(MAX_PAGES + 1, total_pages + 1)
print(f"Pulling pages 1 to {end_page - 1}")

all_dfs = []

for start in range(1, end_page, BATCH_SIZE):
    end = min(start + BATCH_SIZE, end_page)
