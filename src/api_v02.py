#  get all 1_195_392 rows- slow. Working on V3.
...
def get_data():
    ...
    while True:
        ...
        print(f"  Got {len(data['data'])} records. Total so far: {len(all_cats)}/{data['meta']['count']}")

        if len(data['data']) < data['meta']['limit']:
            print('Retrieved all available rows')
            break
